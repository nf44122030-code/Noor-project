...
class _VideoSessionPageState extends State<VideoSessionPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _endSession() async {
    ...
    try {
      ...
      await _syncAndGenerateNotes(isRemoteEnd: false);
    } catch (e) {
  ...
  Future<void> _forceEndSessionFromRemote() async {
    ...
    try {
      await AgoraService.leaveChannel();
      await _syncAndGenerateNotes(isRemoteEnd: true);
    } catch (e) {
      if (mounted) context.pop();
    }
  }

  Future<void> _syncAndGenerateNotes({required bool isRemoteEnd}) async {
    if (_channelId.isEmpty) return;

    String finalNotes = "";
    try {
      // 1. Check if notes already exist in Firestore (generated by the other party)
      final doc = await FirebaseFirestore.instance.collection('bookings').doc(_channelId).get();
      if (doc.exists && doc.data()?['ai_notes'] != null) {
        finalNotes = doc.data()!['ai_notes'];
        debugPrint('📋 Using existing AI notes from Firestore');
      } else {
        ...
        AgoraService.addTranscriptSnippet(isRemoteEnd 
            ? 'Session was ended by the other participant.' 
            : 'Session ended after ${_formatTime(_sessionTime)} with $_expertName.');

        final generated = await AgoraService.generateSessionNotes(_sessionTime, _partialText);
        finalNotes = generated ?? "No discussion notes generated.";
        
        // 3. Save to Firestore for the other party
        if (finalNotes.isNotEmpty && !finalNotes.contains("No conversation data")) {
          await FirebaseFirestore.instance.collection('bookings').doc(_channelId).update({'ai_notes': finalNotes});
        }
      }

      // 4. Save locally to NotesController (using _channelId as the ID for sync)
      if (mounted) {
